...
#%%
class PairsTrading_2(bt.Strategy):
    params = (
        ("window", None),
        ("std_dev", None),
        ("size", None)
    ...
    def __init__(self):
        self.data_a = self.datas[0].close
        self.data_b = self.datas[1].close
        self.data_c = self.datas[2].close
    ...
    def calc_hedge_ratio(self, data_a, data_b, data_c):
        ...
        try:
            ...
            result = coint_johansen(np.array([data_a, data_b, data_c]).T, det_order=0, k_ar_diff=1)
...
if __name__ == "__main__":
    ...
    data_df0 = fetch_data(tickers_pairs[0], '1h', str(days * 24), client)
    data_df1 = fetch_data(tickers_pairs[1], '1h', str(days * 24), client)
    data_df2 = fetch_data(tickers_pairs[2], '1h', str(days * 24), client)

    data0 = bt.feeds.PandasData(dataname=pd.DataFrame(data_df0))
    data1 = bt.feeds.PandasData(dataname=pd.DataFrame(data_df1))
    data2 = bt.feeds.PandasData(dataname=pd.DataFrame(data_df2))
    cerebro.adddata(data0)
    cerebro.adddata(data1)
    cerebro.adddata(data2)

    cerebro.addstrategy(PairsTrading_2, window=window, std_dev=std_dev, size=size)
